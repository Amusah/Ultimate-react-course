...
function reducer(state, action) {
  ...
  switch (type) {
    ...
    case "closeAccount":
      ...
      return { ...state, info: "Account Deactivated", isActive: false };
...
function App() {
  const [{ balance, loan, isActive, info }, dispatch] = useReducer(
    reducer,
    initialState
  );
...
the money is taken from the balance, and the 'loan' 
will get back to 0. This can lead to negative balances, 
but that's no problem, because the customer can't 
